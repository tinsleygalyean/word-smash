...
**Status:** v1.6 · **Last updated:** 2026-08-19
...
- **Launch URL carries query parameters.** The container launches your game as:
  ...
  | `cr_book` | The core-content slug (e.g. a book) — only for existing legacy 3-tier games (§5.1, Layout B). |
...
- One icon per tile — i.e. per (engine, language), or, for existing legacy
  3-tier games, per (engine, core, language).
...
New games split content into two tiers so a shared engine is shipped once and
languages are added as small incremental packs. Existing legacy systems may
also have a third, language-agnostic core tier:

| Tier | What it is |
|---|---|
| **engine** | The main game engine / runtime (top tier, always present) |
| **core** | **Legacy Layout B only:** a language-agnostic mid tier used e.g. for books, where each book's art/structure is shared across its languages |
...
> `-core` filename token, and the legacy **core** ZIP uses the `-book-` token.
...
Two layouts are supported, but **all new games must use Layout A**:
...
> **Layout B is legacy and deprecated for new work.** The 3-tier book pathway
> (`-book-` tokens, `cr_book`, `bookSlug`, and `upload_book`) is retained only
> to maintain existing legacy systems and may be removed in the future. **Do
> not use it for a new game; use the 2-tier Layout A core pathway instead.**

**Layout B — legacy 3-tier (book-style).** One tile per (engine, core,
language), retained for existing engines that play multiple language-agnostic
content units (e.g. books) with per-language variants. The core slug travels
on the wire as `cr_book` / `bookSlug` (frozen physical names).
...
- The tokens `-core` (engine tier), `-lang-<langCode>` (lang tier), and, for
  existing legacy Layout B systems, `-book-<coreSlug>` (core tier), are
  **structural** — the container's tooling classifies ZIPs by these patterns.
  Do not deviate.
- In existing legacy Layout B systems, `<coreSlug>` identifies the core-tier
...
- **One ZIP per language** (or, for existing legacy Layout B systems, per
  (core, language)). Never combine multiple languages in one ZIP.
...
- The core ZIP (legacy Layout B only) is extracted once per (engine, core unit).
...
For legacy Layout B only, the core ZIP carries the content unit's
...
rule. New games must use Layout A instead.
...
- **Legacy Layout B only:** `https://yourplayer.example.org/?cr_book={book}&cr_lang={lang}`

Only the **query part** matters for the offline container — `{lang}` (and,
for legacy Layout B only, `{book}`, the core-unit slug) is substituted per
...
- [ ] Each lang ZIP contains only `lang/<langCode>/…` (or, for an existing legacy Layout B system, its per-(core, language) subtree)
...
| `upload_core_game` | Upload an engine ZIP; optional `versionNotes` |
| `upload_book` | **Legacy only:** upload an existing Layout B core ZIP (`<engine>-book-<slug>.zip`); optional `bookTitle`, `filename`, `versionNotes`; inline `zipBase64` only |
...
> For the recommended Layout A flow, ZIP contents travel either
...
> of `zipBase64`. Chunked files have a **200 MB** ceiling. The legacy-only
...
- `hasCoreLevel`: use `false` for all new games. `true` is only for maintaining
  an existing legacy Layout B (3-tier) engine that already uses the core tier;
  its `urlTemplate` includes `{book}` (e.g.
...
### 8.4 Legacy only — upload an existing Layout B core ZIP (`upload_book`)

> **Deprecated pathway.** `upload_book` exists only to maintain content for an
> existing Layout B system. **Never use it to start a new game**; new games use
> Layout A and skip this step entirely.

One call per existing core unit (e.g. per book). This uploads the
...
- Arguments are `engineSlug`, `bookSlug`, and `zipBase64`, with optional
  `bookTitle`, `filename`, and `versionNotes`. This legacy tool currently
...
One call per language. For an existing legacy Layout B system only, call once
per (core, language) and pass the core-unit slug as `bookSlug` (the frozen
wire name). New games must omit `bookSlug` and use Layout A.
...
- For an **existing legacy Layout B** system only, add
  `"bookSlug": "my-first-book"` — this registers the core unit and the
  (engine, core, language) tile. Do not use this pathway for new games.
...
2. Ask the Curious Learning team to preview the content in their admin CMS
   ...
   be listed with its `zips[]` (`core` + `lang-<code>` for Layout A; an
   existing legacy Layout B tile also includes `book-<slug>`).
...
version; an existing legacy Layout B tile also requires its core tier in
production.
...
| Upload core ZIP (legacy Layout B mid tier) | `POST /api/content/book` | multipart: `file` + `engineSlug`, `bookSlug` (the core-unit slug; + `bookTitle`) |
| Upload language pack | `POST /api/content/lang` | multipart: `file` (+ `icon` PNG) + `engineSlug`, `langCode` (+ `bookSlug` for an existing legacy Layout B system) |
...
4. Read `cr_lang` and `cr_user_id` from `window.location.search`; existing legacy Layout B systems also read `cr_book`.
5. Per-language audio under `lang/<code>/audios/`; MP3/WAV; relative paths only, including inside JSON data.
6. Package new games as Layout A: `<engine>-core.zip` (engine tier, `index.html` at root, no `lang/`) + one `<engine>-lang-<code>.zip` per language (only `lang/<code>/…`). The 3-tier Layout B `-book-` format is legacy-only, retained to maintain existing systems and may be removed.
...
10. Upload a new Layout A game via MCP (`/mcp`, Bearer personal token from the CMS "MCP tokens" tab — shown once at creation; "upload only" scope suffices for uploads, "edit" scope needed to promote): `upload_core_game` → `upload_language_pack` (×N, optionally with `versionNotes`) → `list_inventory` → `promote_content`. For ZIPs over ~10 MB, use `begin_upload` → sequential `upload_chunk` calls (0-based indexes; 4–8 MB recommended, 16 MB decoded maximum) → pass `uploadId` (+ optional `sha256`) to `upload_core_game` or `upload_language_pack` (§8.1a; 200 MB ceiling; abort unused sessions with `abort_upload`). The Layout B book pathway, including `upload_book`, is deprecated and retained only for existing systems; `upload_book` takes inline `zipBase64` only. REST/admin UI are alternatives. Language codes must already exist in the CMS catalog.
